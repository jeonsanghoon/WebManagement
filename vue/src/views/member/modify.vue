...
export default {
  ...
  methods: {
    ...
    memberIdChange: function(val) {
      console.log(val)
      /* BaseCommon.restful.call('/member/getList', { memberId: val })
        .then(res => {
    ...
    uploadCallback: function(file, response, name) {
      console.log(name)
      console.log(file)
      console.log(response)

      if (response.length > 0) {
        this.formData.memberImg = response[0].fileName
        this.formData.memberImgsize = file.size
      }
    },
  },
  mounted() {
